...
def __getHashCodeForBlocks(self, stat: os.stat_result, path: Path) -> str:
  return __md5Parts(self.blockSize, self.blockCount, stat, path)

def getHashcode(self, path: Path) -> str:
  stat = path.stat()
  if (stat.st_size <= self.blockSize * self.blockCount):
    return __getHashCodeAllFile(path)
  else:
    return __getHashCodeForBlocks(self.blockSize, self.blockCount, stat, path)
